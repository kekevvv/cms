...
export default {
    ...
    methods:{
        // 获取公告列表
        fetchNotices(){
            let url = `rest/clubnotice?currentPage=${this.currentPage}&pageSize=${this.pageSize}`
        ...
        // 编辑
        edit(noticeId){
            this.$router.push(`/add_notice/${this.id}?noticeId=${noticeId}`)
        },
        // 删除
        del(noticeId) {
            this.$confirm('确认删除该公告吗', '提示', {
                ...
                this.$http.delete(`rest/clubnotice/${noticeId}`)
                this.$message({
                    type:'success',
                    message:'删除成功'
                })
